...
local _newcclosure = type(newcclosure) == "function" and newcclosure or function(fn) return fn end
local _cloneFunction = type(clonefunction) == "function" and clonefunction or function(fn) return fn end

local _stbl
if type(hookfunction) == "function" and type(getrenv) == "function" and type(getrenv().setmetatable) == "function" then
    _stbl = hookfunction(getrenv().setmetatable, _newcclosure(function(tbl, mt)
...
if not LocalPlayer then
    warn("Arch Script: LocalPlayer is not available yet.")
    return
end

local function _safeWait(parent, name, timeout)
    if not parent or type(parent.WaitForChild) ~= "function" then return nil end
    local ok, obj = pcall(parent.WaitForChild, parent, name, timeout)
    return ok and obj or nil
end

local function _safeRequire(mod)
    if not mod or (typeof(mod) ~= "Instance" and type(mod) ~= "string") then return nil end
    local ok, result = pcall(require, mod)
    return ok and result or nil
end

local PlayerScripts = _safeWait(LocalPlayer, "PlayerScripts", 10)
local Controllers = _safeWait(PlayerScripts, "Controllers", 10)
local Modules = _safeWait(ReplicatedStorage, "Modules", 10)
if not PlayerScripts or not Controllers or not Modules then
    warn("Arch Script: required game modules were not found; script stopped safely.")
    return
end

local CosmeticLibrary = _safeRequire(_safeWait(Modules, "CosmeticLibrary", 10))
local FighterController = _safeRequire(_safeWait(Controllers, "FighterController", 10))
local PlayerDataController = _safeRequire(_safeWait(Controllers, "PlayerDataController", 10))
if type(CosmeticLibrary) ~= "table" or type(PlayerDataController) ~= "table" then
    warn("Arch Script: required module API is unavailable; script stopped safely.")
    return
end
...
local _enumLib = _safeRequire(_safeWait(_mods, "EnumLibrary", 10))
if _enumLib then pcall(function() _enumLib:WaitForEnumBuilder() end) end

local _cosLib  = CosmeticLibrary
if type(_cosLib.Cosmetics) ~= "table" then _cosLib.Cosmetics = {} end
local _itmLib  = _safeRequire(_safeWait(_mods, "ItemLibrary", 10))
...
--// NIL-SAFETY PATCH: defensive helpers for optional Roblox/executor objects
local function _isTable(v) return type(v) == "table" end
local function _isFunction(v) return type(v) == "function" end
local function _safeCamera()
    local cam = workspace and workspace.CurrentCamera
    if cam and cam:IsA("Camera") then return cam end
    return nil
end
local function _safeAntiAimSettings()
    if type(_G.AntiAimSettings) ~= "table" then
        _G.AntiAimSettings = {}
    end
    return _G.AntiAimSettings
end

local _origOwns = _cosLib.OwnsCosmetic
_cosLib.OwnsCosmetic = function(self, inv, nm, wep)
    if type(nm) ~= "string" then
        if _isFunction(_origOwns) then return _origOwns(self, inv, nm, wep) end
        return false
    end
...
_datCtrl.Get = function(self, key)
    local _val = _isFunction(_origGet) and _origGet(self, key) or nil
...
_datCtrl.GetWeaponData = function(self, wn)
    if not _isFunction(_origGetWep) then return nil end
...
local _cliItem
pcall(function()
    local a = _lp and _lp.PlayerScripts and _lp.PlayerScripts:FindFirstChild("Modules")
    local b = a and a:FindFirstChild("ClientReplicatedClasses")
    local c = b and b:FindFirstChild("ClientFighter")
    local d = c and c:FindFirstChild("ClientItem")
    _cliItem = _safeRequire(d)
end)
...
if _cliItem and _cliItem._CreateViewModel then
    ...
    _cliItem._CreateViewModel = function(self, vmRef)
        ...
        if _wp == _lp and _eq[_wn] then
            ...
            if _isTable(vmRef) and _dk ~= nil and vmRef[_dk] then
            ...
            elseif _isTable(vmRef) and _isTable(vmRef.Data) then
        ...
        if not _isFunction(_origCVM) then _buildingWep = nil; return nil end
...
local _clientItemFolder = _lp and _lp.PlayerScripts and _lp.PlayerScripts:FindFirstChild("Modules")
local _clientRepClasses = _clientItemFolder and _clientItemFolder:FindFirstChild("ClientReplicatedClasses")
local _clientFighterFolder = _clientRepClasses and _clientRepClasses:FindFirstChild("ClientFighter")
local _clientItemModule = _clientFighterFolder and _clientFighterFolder:FindFirstChild("ClientItem")
local _vmMod = _clientItemModule and _clientItemModule:FindFirstChild("ClientViewModel")
if _vmMod then
    local _CVM = _safeRequire(_vmMod)
    if type(_CVM) ~= "table" then _CVM = nil end
    local _origNew = _CVM and _CVM.new
    _CVM.new = function(repData, cliItm)
        if not _isTable(repData) then repData = {} end
        if cliItm == nil then
            if _isFunction(_origNew) then return _origNew(repData, cliItm) end
            return nil
        end
            ...
            if katana and type(katana) == "table" and katana.StartAiming then
                ...
                katana.StartAiming = function(self, force)
                    ...
                    if player then
                        ...
                        local dur = (type(info) == "table" and tonumber(info.DeflectDuration)) or 0.6
...
_getgenv().SetAntiKatana = AntiKatanaModule.setAntiKatana
_getgenv().IsKatanaDeflecting = AntiKatanaModule.katanadeflect
...
do
    ...
    _getgenv().InstanceFlushMovementState = flushAntiAimMovementState
    ...
    function if _isFunction(_G.StartAntiAimExt) then _G.StartAntiAimExt() end
    ...
    function if _isFunction(_G.StopAntiAimExt) then _G.StopAntiAimExt() end
    ...
    function _G.SetUndergroundExt(state)
        ...
        _getgenv().InstanceUndergroundEnabled = state
        if state then
...
local Mouse = (type(Player.GetMouse) == "function") and Player:GetMouse() or nil
local _initialCamera = _safeCamera()
local ViewportSize = _initialCamera and _initialCamera.ViewportSize or Vector2.new(800, 600)
...
local function UpdateScale()
    local cam = _safeCamera()
    if not cam or not UIScale then return end
...
local Workspace = game:GetService("Workspace")
local Camera = _safeCamera() or workspace.CurrentCamera
local _cameraRefreshConn = RunService.RenderStepped:Connect(function()
    local cam = _safeCamera()
...
local S = {
    ...
    -- Emote state (ported from pasted EmoteUI)
    emoteEnabled = false,
    emoteAnimationId = "",
    emoteLoop = true,
    emoteSpeed = 2,
    emoteSpawnProof = true,
    emotePresetName = nil,
}
...
-- ============================================================
-- Emote module
-- Exact Emote logic ported from the supplied Eclipse-style Emote UI.
-- The standalone Emote ScreenGui is NOT created; Arch's native Library
-- is used for the controls so only one main UI remains active.
-- ============================================================
...
-- @param char Instance|nil Character model
-- @return Animator|nil
...
-- @param id string|nil Animation asset id
-- @return Animation|nil
local function resolveAnim(id)
    if type(id) ~= "string" then return nil end

    local cleanId = id:match("%d+") or ""
    ...
    if ok and type(objects) == "table" then
        for _, obj in ipairs(objects) do
            if typeof(obj) == "Instance" and obj:IsA("Animation") and obj.AnimationId ~= "" then
                return obj
            end
        end
...
-- @param char Instance|nil Character model
-- @param id string|nil Animation asset id
-- @param speed number|nil Playback speed
-- @param looped boolean|nil Whether the animation loops
-- @return AnimationTrack|nil
local function loadPlay(char, id, speed, looped)
    if not char or typeof(char) ~= "Instance" then return nil end
    ...
    local animation = resolveAnim(id)
...
local function stopAll()
    if type(activeTracks) ~= "table" then
        activeTracks = {}
        return
    end
...
local function playEmote()
    ...
    if S.emoteEnabled ~= true then return end
    if type(S.emoteAnimationId) ~= "string" or S.emoteAnimationId == "" then return end

    local char = LocalPlayer and LocalPlayer.Character
    if char then
        local track = loadPlay(
            char,
            S.emoteAnimationId,
            S.emoteSpeed,
            S.emoteLoop
    ...
    local live = Workspace and Workspace:FindFirstChild("Live")
    if live and LocalPlayer then
...
-- @param name string|nil Preset name
-- @return boolean True when a valid preset was selected
local function selectEmotePreset(name)
    if type(name) ~= "string" or type(S) ~= "table" then return false end

    local id = animPresets[name]
    if type(id) ~= "string" or id == "" then return false end

    S.emoteEnabled = true
    S.emotePresetName = name
    S.emoteAnimationId = id
    playEmote()
    return true
end

-- @param id string|nil Animation asset id
-- @return boolean True when the supplied id was accepted
local function playCustomEmote(id)
    if type(id) ~= "string" or type(S) ~= "table" then return false end

    local cleanId = id:match("%d+") or ""
    if cleanId == "" then return false end

    S.emoteEnabled = true
    S.emotePresetName = nil
    S.emoteAnimationId = cleanId
    playEmote()
    return true
end
...
RunService.Heartbeat:Connect(function()
    if type(S) ~= "table" or S.emoteEnabled ~= true then return end

    if #activeTracks == 0 and type(S.emoteAnimationId) == "string" and S.emoteAnimationId ~= "" then
        playEmote()
        return
    end
    ...
    for _, track in ipairs(activeTracks) do
        if track then
            pcall(function()
                track:AdjustSpeed(tonumber(S.emoteSpeed) or 1)
            end)
        end
    end
end)
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
    if type(S) == "table" and S.emoteEnabled == true and S.emoteSpawnProof == true then
        playEmote()
    end
end)
...
local function toggleUnderground(on)
    ...
    if _isFunction(_G.SetUndergroundExt) then _G.SetUndergroundExt(on) end
    if on then
        if _isFunction(_G.SetUndergroundDepthExt) then _G.SetUndergroundDepthExt(S.undergroundDepth) end
    end
end
        ...
        aimbotConn = RunService.RenderStepped:Connect(function(dt)
            ...
            if targetPart then
                ...
                if S.aimSmooth <= 0.01 then
                    ...
                    local newCF = cam.CFrame:Lerp(targetCF, math.clamp(smoothFactor, 0, 1))
                    if CameraControllerAimbot and CameraControllerAimbot.MimicRotation then
                        pcall(function()
...
local function saveOriginalGunFunctions()
    local ok, Gun = pcall(function()
        local m = LocalPlayer and LocalPlayer.PlayerScripts and LocalPlayer.PlayerScripts:FindFirstChild("Modules")
        local t = m and m:FindFirstChild("ItemTypes")
        local g = t and t:FindFirstChild("Gun")
        return _safeRequire(g)
    end)
...
local function applyGunEnhancements()
    ...
    if S.noEquipAnimEnabled then
        Gun.Equip = function(self, ...)
            if not _isFunction(originalGunFunctions.Equip) then return nil end
    ...
    if S.silentAimActive or S.noShootAnimEnabled then
        ...
        Gun.StartShooting = function(self, p1, p2)
            if S.silentAimActive then
                local cam = _safeCamera()
                if not cam then
                    if _isFunction(currentShooting) then return currentShooting(self, p1, p2) end
                    return nil
                end
                ...
                if targetPartInstance then
                    if _getgenv().IsKatanaDeflecting and _getgenv().IsKatanaDeflecting(targetPlayer) then
                        return currentShooting(self, p1, p2)
                    end
                    local oldCamCF = cam.CFrame
                    cam.CFrame = CFrame.lookAt(camPos, targetPartInstance.Position)
...
function ApplyAllSettings()
    ...
    stopIrregularMove()
    stopTriggerbot()
    if _isFunction(_G.StopAntiAimExt) then _G.StopAntiAimExt() end
    ...
    -- Restore saved Emote state after config load.
    if S.emoteEnabled and S.emoteAnimationId ~= "" then
        task.defer(playEmote)
    end

    if _safeAntiAimSettings().enabled then
        if _isFunction(_G.StartAntiAimExt) then _G.StartAntiAimExt() end
    end
...
local function buildAim(base, pitch, oy, oz)
    if type(base) ~= "table" then base = AIM_EXTRA or { ["\0"] = 0, ["\1"] = 0, ["\2"] = 0 } end
    return { ["\0"] = base["\0"] or 0, ["\1"] = base["\1"] or 0, ["\2"] = base["\2"] or 0, ["\3"] = pitch or 0, ["\4"] = oy or 0, ["\5"] = oz or 0 }
end
...
local fireServerNative = _cloneFunction(cleanRemote.FireServer)

-- ---- bypass helpers (ported from KiciaHook t1141) ----------------------------------------
local rbCleanFireEvent = Instance.new("RemoteEvent")
local rbFireServerNative = _cloneFunction(rbCleanFireEvent.FireServer)
...
function PartGlue:Acquire(ourPart, hitboxPart)
    local prev = (type(getthreadidentity) == "function" and getthreadidentity()) or nil
    ...
    if type(setthreadidentity) == "function" and prev ~= nil then pcall(setthreadidentity, prev) end
...
AntiAimGroup:Dropdown({Name = "Yaw Type", Options = {"none", "jitter", "spinbot", "random"}, Default = "none", Callback = function(v)
    _safeAntiAimSettings().yawtype = v
end})
AntiAimGroup:Dropdown({Name = "Pitch Type", Options = {"none", "jitter", "spinbot", "random"}, Default = "none", Callback = function(v)
    _safeAntiAimSettings().pitchtype = v
end})
AntiAimGroup:Dropdown({Name = "Angle Type", Options = {"none", "tilt 45", "tilt 90", "upside down", "custom"}, Default = "none", Callback = function(v)
    _safeAntiAimSettings().angletype = v
end})
AntiAimGroup:Slider({Name = "Custom Angle", Min = 0, Max = 360, Default = 0, Unit = "Â°", Callback = function(v)
    _safeAntiAimSettings().customangle = v
end})
AntiAimGroup:Slider({Name = "Min Speed", Min = 1, Max = 100, Default = 10, Unit = "", Callback = function(v) _safeAntiAimSettings().minspeed = v end})
AntiAimGroup:Slider({Name = "Max Speed", Min = 1, Max = 100, Default = 20, Unit = "", Callback = function(v) _safeAntiAimSettings().maxspeed = v end})
AntiAimGroup:Slider({Name = "Min Angle", Min = 1, Max = 180, Default = 30, Unit = "Â°", Callback = function(v) _safeAntiAimSettings().minangle = v end})
AntiAimGroup:Slider({Name = "Max Angle", Min = 1, Max = 180, Default = 60, Unit = "Â°", Callback = function(v) _safeAntiAimSettings().maxangle = v end})
AntiAimGroup:Toggle({Name = "Random Angle (Jitter)", Callback = function(v) _safeAntiAimSettings().randomangle = v end})
...
-- ============================================================
-- Emote UI
-- Uses Arch's native Library UI instead of spawning a second
-- standalone ScreenGui.
-- ============================================================

local EmoteTab = Library:Tab("Emote", 3926305904)

local EmotePresetGroup = EmoteTab:Group("Presets")
local activePreset = nil
local presetButtons = {}

local function clearPreset()
    if activePreset and presetButtons[activePreset] then
        local button = presetButtons[activePreset]
        pcall(function()
            Tween(button, {
                BackgroundColor3 = CFG.SecondaryColor,
                TextColor3 = CFG.TextDark,
...
for _, presetName in ipairs(PRESET_ORDER) do
    local presetId = animPresets[presetName]
    if type(presetId) == "string" and presetId ~= "" then
        local button
        button = EmotePresetGroup:Button({
            Name = presetName,
            Tooltip = "Play " .. presetName,
            Callback = function()
                if type(S) == "table" and S.emoteEnabled == true and S.emotePresetName == presetName then
                    stopEmote()
                    clearPreset()
                    return
                end
                ...
                if not selectEmotePreset(presetName) then
                    Library:Notify("Failed to play Emote: " .. presetName, "warning")
                    return
                end

                activePreset = presetName
                presetButtons[presetName] = button
                pcall(function()
                    Tween(button, {
                        BackgroundColor3 = CFG.SecondaryColor,
...
local EmoteCustomGroup = EmoteTab:Group("Custom Emote")

EmoteCustomGroup:Textbox({
    Name = "Asset ID",
    Placeholder = "Animation Asset ID...",
    Callback = function(value)
        S.emoteAnimationId = type(value) == "string" and (value:match("%d+") or "") or ""
        S.emotePresetName = nil
    end
})
...
EmoteCustomGroup:Button({
    ...
    Variant = "Primary",
    Callback = function()
        if not playCustomEmote(S.emoteAnimationId) then
            Library:Notify("Invalid Animation Asset ID", "warning")
        end
    end
})

EmoteCustomGroup:Slider({
    Name = "Speed",
    Min = 1,
    Max = 500,
    Default = 2,
...
local EmoteOptionsGroup = EmoteTab:Group("Options")
...
EmoteOptionsGroup:Toggle({
    Name = "SpawnProof",
    Default = true,
    Tooltip = "Replay the selected Emote after respawn",
    Callback = function(v)
        S.emoteSpawnProof = v == true
    end
})
