...
local S = {
    ...
    boosterAutoHealth = true,
    boosterAutoAmmo = true,
    boosterAutoRespawn = true,
}
...
-- Auto Collect Boosters
local BoosterSupported = (game.CreatorId == 3461453)
local BoosterModified = {}
local BoosterRespawnToken = 0
local BoosterRespawnConnections = {}

local function boosterRestoreObject(obj)
    local state = BoosterModified[obj]
    ...
    if obj and obj.Parent then
        ...
        for child, childState in pairs(state.Children or {}) do
            if child and child.Parent then
                pcall(function()
                    if childState.Transparency ~= nil then child.Transparency = childState.Transparency end
                    if childState.Enabled ~= nil then child.Enabled = childState.Enabled end
                end)
            end
        end
    end
    BoosterModified[obj] = nil
end

local function boosterHideObject(obj, targetCFrame)
    if not obj or not obj.Parent or not obj:IsA("BasePart") then return end
    if not BoosterModified[obj] then
        local state = {
        ...
        for _, child in ipairs(obj:GetDescendants()) do
            if child:IsA("BasePart") or child:IsA("UnionOperation") or child:IsA("MeshPart") or child:IsA("SpecialMesh") then
                state.Children[child] = {Transparency = child.Transparency}
            elseif child:IsA("BillboardGui") or child:IsA("SurfaceGui") or child:IsA("ParticleEmitter") or child:IsA("SelectionBox") then
                state.Children[child] = {Enabled = child.Enabled}
            end
        end
        BoosterModified[obj] = state
    end
...
local function boosterCollectStep()
    if not BoosterSupported then return end
    ...
    local targetCFrame = CFrame.new(hrp.Position + Vector3.new(0, bounce, 0))

    for _, obj in ipairs(workspace:GetChildren()) do
        if obj and obj.Name == "_drop" and obj:IsA("BasePart") then
            local isAmmo = S.boosterAutoAmmo and obj:FindFirstChild("Ammo") ~= nil
            local isHealth = S.boosterAutoHealth and obj:FindFirstChild("Health") ~= nil
            if isAmmo or isHealth then
                boosterHideObject(obj, targetCFrame)
            end
        end
    end
end

local function boosterRestoreAll()
    for obj in pairs(BoosterModified) do
        boosterRestoreObject(obj)
    end
end

local function boosterBindCharacter(character)
    if not character then return end
    for _, conn in pairs(BoosterRespawnConnections) do
        if conn then pcall(function() conn:Disconnect() end) end
    end
    BoosterRespawnConnections = {}

    local humanoid = character:FindFirstChildOfClass("Humanoid") or character:WaitForChild("Humanoid", 10)
    if not humanoid then return end

    BoosterRespawnConnections.Died = humanoid.Died:Connect(function()
        if not S.boosterAutoRespawn or not BoosterSupported then return end
        BoosterRespawnToken = BoosterRespawnToken + 1
        local token = BoosterRespawnToken
        task.spawn(function()
            while S.boosterAutoRespawn and BoosterSupported and token == BoosterRespawnToken do
...
if BoosterSupported then
    if LocalPlayer.Character then
        boosterBindCharacter(LocalPlayer.Character)
    end
    LocalPlayer.CharacterAdded:Connect(boosterBindCharacter)
end

local BoosterConnection = RunService.RenderStepped:Connect(function()
    pcall(boosterCollectStep)
end)

local BoosterGroup = Visuals:Group("Boosters")
BoosterGroup:Toggle({
    Name = "Auto Health",
    Default = S.boosterAutoHealth,
    Tooltip = "Automatically collect _drop objects containing Health",
    Callback = function(v)
        S.boosterAutoHealth = (v == true)
        if not S.boosterAutoHealth and not S.boosterAutoAmmo then
            boosterRestoreAll()
        end
    end
})
BoosterGroup:Toggle({
    Name = "Auto Ammo",
    Default = S.boosterAutoAmmo,
    Tooltip = "Automatically collect _drop objects containing Ammo",
    Callback = function(v)
        S.boosterAutoAmmo = (v == true)
        if not S.boosterAutoHealth and not S.boosterAutoAmmo then
            boosterRestoreAll()
        end
    end
...
BoosterGroup:Toggle({
    Name = "Auto Respawn",
    Default = S.boosterAutoRespawn,
    Tooltip = "Attempts automatic respawn using the Space key",
    Callback = function(v)
        S.boosterAutoRespawn = (v == true)
        if not S.boosterAutoRespawn then
            BoosterRespawnToken = BoosterRespawnToken + 1
        end
    end
})
...
local function EmoteGetAnimator(character)
    ...
    if not animator then
        local ok, created = pcall(function()
            local value = Instance.new("Animator")
...
local function EmoteResolveAnimation(id)
        ...
        for _, object in ipairs(objects) do
            if object then
                for _, child in ipairs(object:GetDescendants()) do
                    if child and child:IsA("Animation") and child.AnimationId ~= "" then
                        return child
                    end
                end
            end
...
local function EmoteLoadPlay(character, id, speed, looped)
    if not character or type(id) ~= "string" or id == "" then
        return nil
    end

    local animator = EmoteGetAnimator(character)
    if not animator then return nil end

    local animation = EmoteResolveAnimation(id)
...
local function EmotePlay()
    ...
    if type(EmoteState.AnimationId) ~= "string" or EmoteState.AnimationId == "" then return end

    local character = LocalPlayer and LocalPlayer.Character
    if character then
        local track = EmoteLoadPlay(
            character,
    ...
    if live and LocalPlayer then
        local replicatedCharacter = live:FindFirstChild(LocalPlayer.Name)
        if replicatedCharacter then
            local track = EmoteLoadPlay(
                replicatedCharacter,
                EmoteState.AnimationId,
                EmoteState.Speed,
...
EmoteConnections.Heartbeat = RunService.Heartbeat:Connect(function()
    if not EmoteState.Enabled then return end

    if #EmoteTracks == 0 then
    ...
    for _, track in ipairs(EmoteTracks) do
        if track then
            validTracks += 1
            pcall(function()
                track:AdjustSpeed(tonumber(EmoteState.Speed) or 1)
            end)
        end
    end
...
if LocalPlayer then
    EmoteConnections.CharacterAdded = LocalPlayer.CharacterAdded:Connect(function()
        task.wait(0.5)
...
local EmoteCustomGroup = EmoteTab:Group("Custom")
local EmoteOptionsGroup = EmoteTab:Group("Options")

local EmoteOptions = {}
for _, name in ipairs(EmotePresetOrder) do
    if type(name) == "string" and name ~= "" and type(EmoteAnimations[name]) == "string" then
        table.insert(EmoteOptions, name)
    end
end

if #EmoteOptions > 0 then
    EmotePresetGroup:Dropdown({
        Name = "Emote",
        Options = EmoteOptions,
        Default = EmoteOptions[1],
        Tooltip = "Select an emote animation",
        Callback = function(name)
            if type(name) ~= "string" or name == "" then
                return
            end

            local animationId = EmoteAnimations[name]
            if type(animationId) ~= "string" or animationId == "" then
                Library:Notify("Invalid emote animation", "warning")
                return
            end

            EmoteState.Enabled = true
            EmoteState.PresetName = name
            EmoteState.AnimationId = animationId

            EmotePlay()
            Library:Notify("Playing: " .. tostring(name), "success")
        end,
    })
end

local EmoteCustomID = EmoteCustomGroup:Textbox({
    Name = "Animation ID",
...
EmoteCustomGroup:Button({
    ...
    Callback = function()
        local raw = ""
        if EmoteCustomID and type(EmoteCustomID.Get) == "function" then
            local ok, value = pcall(function()
                return EmoteCustomID:Get()
            end)
...
if BoosterConnection then
    task.spawn(function()
        ...
        pcall(function() BoosterConnection:Disconnect() end)
        boosterRestoreAll()
        BoosterRespawnToken = BoosterRespawnToken + 1
    end)
end
